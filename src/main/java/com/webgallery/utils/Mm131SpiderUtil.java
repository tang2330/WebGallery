...
public class Mm131SpiderUtil {
    ...
    public static List<String> readFileForCheck(String pathName, List<String> urlList) {
        ...
        if(file.exists()) {
            ...
            fileReader.readLines().forEach(urlList::remove);
        }
    ...
    public static byte[] getImageFromNetByUrl(String strUrl) throws IOException {
        ...
        try {
            ...
            conn.setRequestProperty("User-Agent", RandomUtil.randomEle(Mm131SpiderConfig.USER_AGENT_LIST));
            // 很多网站的防采集的办法,就是判断浏览器来源referer和cookie以及userAgent
            conn.setRequestProperty("Referer", Mm131SpiderConfig.DOMAIN);
            ...
            String encode = Base64.encode(btImg);
